...
from fastapi import APIRouter, Depends, HTTPException, status, Header, Request, BackgroundTasks, Query
...
from app.dependencies.auth_deps import get_current_active_user, get_optional_user, require_manager_or_above
...
async def verify_payment(
    ...
    if settings.PAYSTACK_SECRET_KEY == "sk_test_mock_secret_key" or reference.startswith("mock"):
        tx.status = "success"
        ...
        background_tasks.add_task(EmailService.send_order_confirmation, email=order.customer_email, order=order)
    ...
    if data["status"] == "success" and data["amount"] == tx.amount * 100:
        ...
        await db.commit()
        return {"status": "failed", "detail": data.get("gateway_response", "Payment failed")}
